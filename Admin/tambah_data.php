<?php
    require_once __DIR__ . "/../config/database.php";
    session_start();

    // Query untuk opsi Kategori
    $sql_kategori = mysqli_query($conn, "SELECT id_kategori, nama_kategori FROM kategori ORDER BY nama_kategori ASC");
    if (!$sql_kategori) {
        die("Query kategori error: " . mysqli_error($conn));
    }

    $create_message = "";
    $message_type = "";

    // Proses Tambah Data
    if (isset($_POST["tambah_data"])) {
        $id_kategori = $_POST["id_kategori"];
        $nama_barang = $_POST["nama_barang"];
        $merk        = $_POST["merk"];
        $tipe        = $_POST["tipe"];
        $spesifikasi = $_POST["spesifikasi"];
        $tersedia    = $_POST["tersedia"];

        // Menggunakan Prepared Statement untuk keamanan dari SQL Injection
        $stmt = mysqli_prepare($conn, "INSERT INTO barang (id_kategori, nama_barang, merk, tipe, spesifikasi, tersedia) VALUES (?, ?, ?, ?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "isssss", $id_kategori, $nama_barang, $merk, $tipe, $spesifikasi, $tersedia);

        if (mysqli_stmt_execute($stmt)) {
            header("Location: inventaris.php?tambah_data=success");
            exit;
        } else {
            $create_message = "Data gagal ditambahkan: " . mysqli_error($conn);
            $message_type = "error";
        }
        mysqli_stmt_close($stmt);
    }
?>
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>InventarisApp - Tambah Barang</title>
    
    <!-- Tailwind CSS -->
    <script src="https://cdn.tailwindcss.com"></script>
    
    <!-- Font Awesome -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    
    <!-- Custom Stylesheet -->
    <link rel="stylesheet" href="../assets/styl_tambah_data.css">
    
    <style>
        aside { transition: width 0.3s ease; }
        .collapsed {
            width: 0 !important;
            overflow: hidden;
        }
    </style>
</head>
<body class="bg-gray-100 font-sans">
    <div class="flex h-screen overflow-hidden">
        <?php include '../include/menu.php'; ?>
        
        <div class="flex-1 flex flex-col overflow-y-auto">
            <?php include '../include/header_hlm.php'; ?>
            
            <div class="p-6">
                <!-- Alert Success -->
                <?php if (isset($_GET['tambah_data']) && $_GET['tambah_data'] == 'success'): ?>
                    <div class="bg-green-500 text-white p-3 rounded mb-4 text-center shadow-sm" id="alert">
                        <i class="fas fa-check-circle mr-2"></i> Barang Berhasil Ditambahkan!
                    </div>
                <?php endif; ?>

                <!-- Alert Error -->
                <?php if (!empty($create_message)): ?>
                    <div class="bg-red-500 text-white p-3 rounded mb-4 text-center shadow-sm" id="alert">
                        <i class="fas fa-exclamation-circle mr-2"></i> <?= $create_message; ?>
                    </div>
                <?php endif; ?>

                <div class="mb-4 flex justify-between items-center">
                    <div>
                        <h1 class="text-2xl font-semibold text-gray-800">Tambah Inventaris Barang</h1>
                        <p class="text-sm text-gray-500">Masukkan detail barang baru ke dalam sistem.</p>
                    </div>
                    <button onclick="bukaModal()" class="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded text-sm font-semibold shadow-sm transition flex items-center">
                        <i class="fas fa-plus-circle mr-2"></i> Kategori Baru
                    </button>
                </div>

                <!-- Form Card -->
                <div class="bg-white rounded shadow-md border-t-4 border-[#3c8dbc]">
                    <div class="px-6 py-4 border-b border-gray-100 flex items-center">
                        <i class="fas fa-box-open text-[#3c8dbc] mr-2"></i>
                        <h3 class="font-bold text-gray-700 uppercase tracking-wider text-sm">Form Detail Barang</h3>
                    </div>

                    <form method="POST" autocomplete="off" class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">

                            <!-- 1. Kategori (id_kategori) -->
                            <div class="mb-2">
                                <label class="block text-sm font-bold text-gray-700 mb-1">Kategori <span class="text-red-500">*</span></label>
                                <select name="id_kategori" required class="w-full border border-gray-300 p-2 rounded shadow-sm focus:border-[#3c8dbc] focus:ring focus:ring-[#3c8dbc]/20">
                                    <option value="">-- Pilih Kategori --</option>
                                    <?php while ($k = mysqli_fetch_assoc($sql_kategori)): ?>
                                        <option value="<?= $k['id_kategori']; ?>"><?= $k['nama_kategori']; ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>

                            <!-- 2. Nama Barang (nama_barang) -->
                            <div class="mb-2">
                                <label class="block text-sm font-bold text-gray-700 mb-1">Nama Barang <span class="text-red-500">*</span></label>
                                <input type="text" name="nama_barang" placeholder="Contoh: Laptop, Monitor, Mouse..." required
                                       class="w-full border border-gray-300 p-2 rounded shadow-sm focus:border-[#3c8dbc] focus:ring focus:ring-[#3c8dbc]/20">
                            </div>

                            <!-- 3. Merk (merk) -->
                            <div class="mb-2">
                                <label class="block text-sm font-bold text-gray-700 mb-1">Merk</label>
                                <input type="text" name="merk" placeholder="Dell, HP, Lenovo..."
                                       class="w-full border border-gray-300 p-2 rounded shadow-sm focus:border-[#3c8dbc] focus:ring focus:ring-[#3c8dbc]/20">
                            </div>

                            <!-- 4. Tipe (tipe) -->
                            <div class="mb-2">
                                <label class="block text-sm font-bold text-gray-700 mb-1">Tipe</label>
                                <input type="text" name="tipe" placeholder="Inspiron 15, ThinkPad X1..."
                                       class="w-full border border-gray-300 p-2 rounded shadow-sm focus:border-[#3c8dbc] focus:ring focus:ring-[#3c8dbc]/20">
                            </div>

                        </div>

                        <!-- 5. Spesifikasi (spesifikasi) -->
                        <div class="mt-4">
                            <label class="block text-sm font-bold text-gray-700 mb-1">Spesifikasi</label>
                            <textarea name="spesifikasi" rows="3" placeholder="Masukkan detail spesifikasi barang..."
                                      class="w-full border border-gray-300 p-2 rounded shadow-sm focus:border-[#3c8dbc] focus:ring focus:ring-[#3c8dbc]/20"></textarea>
                        </div>

                        <!-- 6. Status Tersedia (tersedia) -->
                        <div class="mt-4">
                            <label class="block text-sm font-bold text-gray-700 mb-1">Status Ketersediaan <span class="text-red-500">*</span></label>
                            <select name="tersedia" required class="w-full border border-gray-300 p-2 rounded shadow-sm focus:border-[#3c8dbc] focus:ring focus:ring-[#3c8dbc]/20">
                                <option value="">-- Pilih Status --</option>
                                <option value="Iya">Tersedia (Iya)</option>
                                <option value="Tidak">Tidak Tersedia (Tidak)</option>
                            </select>
                        </div>

                        <!-- Button Actions -->
                        <div class="mt-8 pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
                            <a href="inventaris.php" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded text-sm font-semibold transition flex items-center">
                                <i class="fas fa-arrow-left mr-2"></i> Kembali
                            </a>
                            <button name="tambah_data" type="submit" class="px-4 py-2 bg-[#3c8dbc] hover:bg-[#367fa9] text-white rounded text-sm font-semibold shadow-sm transition flex items-center">
                                <i class="fas fa-save mr-2"></i> Simpan Data Barang
                            </button>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>

    <!-- Modal Tambah Kategori -->
    <div id="modalKategori" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <h4 class="font-bold text-gray-700">Tambah Kategori Baru</h4>
                <button onclick="tutupModal()" class="text-gray-400 hover:text-gray-600 text-2xl">&times;</button>
            </div>
            <form action="proses_kategori.php" method="POST" class="p-6">
                <div class="mb-6">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Nama Kategori</label>
                    <input type="text" name="nama_kategori" placeholder="Elektronik, Furniture, dll..." required 
                           class="w-full border border-gray-300 p-2 rounded shadow-sm focus:border-[#3c8dbc] focus:ring focus:ring-[#3c8dbc]/20">
                </div>
                <div class="flex justify-end gap-2">
                    <button type="button" onclick="tutupModal()" class="px-4 py-2 text-gray-600 hover:text-gray-800 font-semibold text-sm">Batal</button>
                    <button name="simpan_kategori" type="submit" class="px-6 py-2 bg-green-500 hover:bg-green-600 text-white rounded font-semibold text-sm shadow-sm transition">
                        Simpan Kategori
                    </button>
                </div>
            </form>
        </div>
    </div>

    <script>
        const alertBox = document.getElementById('alert');
        if (alertBox) {
            setTimeout(() => {
                // Efek memudar (fade out)
                alertBox.style.opacity = '0';
                
                // Hapus element dari DOM setelah efek pudar selesai (0.5 detik)
                setTimeout(() => {
                    alertBox.remove();
                }, 500);
            }, 3000); // 3000 milidetik = 3 detik
        }
        function bukaModal() {
            document.getElementById('modalKategori').classList.remove('hidden');
        }
        function tutupModal() {
            document.getElementById('modalKategori').classList.add('hidden');
        }
    </script>
</body>
</html>